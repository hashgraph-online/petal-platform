...
export default function Search() {
  const algoliaConfig = useMemo(
    () => ({
      ...
      apiKey:
        ...
        "2b3fe9b2882e46e19fa5fce6272efe4f",
  ...
  return (
    <div
      className={`navbar-search-wrapper ${isExpanded ? "navbar-search-wrapper--active" : ""}`}
      ref={containerRef}
    >
      ...
      {isExpanded && (query || hits.length > 0) && (
        <div className="search-suggestions-panel" role="listbox">
          {loading ? (
              ...
              {hits.map((hit) => (
                ...
                >
                  <a href={hit.url}>
                    ...
                    <div className="search-suggestions-snippet">
                      {getSnippet(hit)}
                    </div>
            ...
            <div className="search-suggestions-empty">
              Type to search Hashgraph Online
            </div>
          )}
        </div>
      )}
